...
do
  ...
  echo "Alter Table $table. File is $t"
...
# ../fwk/schema/ddl_alter_con.sh "$DB" "$tbs_uc"
...
sqlplus -S $DB << __EOF__
  merge into sync_core_settings using dual on (raise_dbms_alerts is not null) when not matched then insert (raise_dbms_alerts) values ('N'); 
  commit;
  show errors;
__EOF__
